...
7. 创建 flows/ 目录，生成初始流程文档:
   - **先创建 L1 主流程文件**（FL-{abbr}-01-{主流程名}.md），按 L1 主流程模板填写：元信息、主流程概览、阶段流转、异常汇总
   - 再为每个阶段创建独立的 L2+ 子流程文件（FL-{abbr}-0N-{子流程名}.md），按通用模板填写
   - 主流程文件是"全貌视图"，只写阶段串联和流转条件，不展开子流程细节
   - 子流程文件通过主流程文件中的引用链接关联回主流程
   ...
   - 生成 flows/INDEX.md（流程列表 + 文件映射，区分标注 L1 主流程）
...
│   ├── FL-{abbr}-01-{主流程名}.md               # ⬅ L1 主流程（独立文件，全貌视图）
│   ├── FL-{abbr}-02-{子流程名}.md               # ⬅ L2+ 子流程（独立文件）
│   └── ...
└── rounds/
    ├── round-N/
    │   ├── plan.md
...
- [关键业务约束或规则，如: 攻城战每日限参与3次]
- [...]

## 验收标准

- [该流程完成的判定条件，如: 攻城结果已结算且奖励已发放]
- [...]
...
**关键要求**: [本步骤必须满足的业务规则/约束，引用 PRD 链接]
**验收标准**: [本步骤完成的判定条件]
...
**L1 主流程必须独立成文件，与子流程文件分开。** 它是整个功能模块的"全貌视图"——阅读者只需看这一个文件，就能理解功能的完整链路，不会在子流程细节中迷失。

| 维度 | L1 主流程 | L2+ 子流程 |
|------|----------|-----------|
| 文件形式 | **独立文件** `FL-{abbr}-01-主流程.md` | 独立文件 `FL-{abbr}-0N-子流程名.md` |
| 步骤粒度 | 阶段/子流程引用 | 具体操作步骤 |
| 用户输入 | 用户提供的线性步骤链 | PRD 细节提取 |
| 核心关注 | 阶段串联、流转条件、异常跳转 | 用户感知、系统行为、字段细节 |
| 全貌作用 | 一眼看清功能完整链路 | 深入某一阶段的实施细节 |
| 展开 | 每个步骤 → 子流程（独立文件） | 每个步骤 → 具体描述 |

**设计目的**: 避免开发者在多个子流程文件之间来回跳转时迷失上下文。主流程文件提供"总-分"导航：先看全貌，再按需进入子流程。
...
显示行军线路 → 等待移动到目标附近 → 切换为攻占动画 → 等待攻占过程 →
攻占结果动画 → 攻占结果结算 → 生成攻占结果事件 → 编队状态更新 →
自动回城 → 军队精灵按线路移动回出发城市
```
...
FL-{abbr}-01 {功能名称}主流程
├── Phase A: {阶段名称}
│   └── → [FL-{abbr}-02](FL-{abbr}-02-{子流程名}.md) {子流程名称}
├── Phase B: {阶段名称}
│   └── → [FL-{abbr}-03](FL-{abbr}-03-{子流程名}.md) {子流程名称}
├── ...
└── Phase N: {阶段名称}
    └── → [FL-{abbr}-0N](FL-{abbr}-0N-{子流程名}.md) {子流程名称}
```
...
**阶段目标**: [此阶段完成什么事情]
**对应子流程**: → [FL-{abbr}-02-{子流程名}.md](FL-{abbr}-02-{子流程名}.md)（独立文件，点击查看详情）
