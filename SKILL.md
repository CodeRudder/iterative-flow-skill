...
7. 创建 flows/ 目录，生成初始流程文档:
   - **先创建 L1 主流程文件**（文件名: `FL-{abbr}-{NN}-{title}-主流程.md`），按 L1 主流程模板填写，列出该功能的所有主流程链路
   - 再为每个阶段创建独立的 L2+ 子流程文件（`FL-{abbr}-{NN}-{title}.md`），按子流程文档模板填写
...
│   ├── FL-{abbr}-{NN}-{title}-主流程.md        # ⬅ L1 主流程（全貌视图，列出所有功能主流程）
│   ├── FL-{abbr}-{NN}-{title}.md               # ⬅ L2+ 子流程（独立文件）
│   └── ...
└── rounds/
    ├── round-N/
...
**L1 主流程必须独立成文件，文件名固定为 `FL-{abbr}-{NN}-{title}-主流程.md`。** 它是整个功能模块的"全貌视图"——一个主流程文件列出该功能相关的所有主流程链路，阅读者只需看这一个文件就能理解完整链路，不会在子流程细节中迷失。

| 维度 | L1 主流程 | L2+ 子流程 |
|------|----------|-----------|
| 文件名 | `FL-{abbr}-{NN}-{title}-主流程.md` | `FL-{abbr}-{NN}-{title}.md` |
| 内容范围 | **列出功能的所有主流程链路** | 单个子流程的详细展开 |
| 步骤粒度 | 阶段/子流程引用 | 具体操作步骤 |
| 用户输入 | 用户提供的线性步骤链 | PRD 细节提取 |
| 核心关注 | 阶段串联、流转条件、异常跳转 | 用户感知、系统行为、时间/空间要点 |
| 全貌作用 | 一眼看清功能完整链路 | 深入某一阶段的实施细节 |
| 展开 | 每个步骤 → 子流程（独立文件） | 每个步骤 → 具体描述 |

**设计目的**: 避免开发者在多个子流程文件之间来回跳转时迷失上下文。主流程文件提供"总-分"导航：先看全貌，再按需进入子流程。

> 完整模板见 [L1 主流程模板](references/L1-main-flow-template.md)
